<x-app-layout>
    <div class="flex justify-center">
        <div
            class="max-w-lg w-full bg-gray-800 shadow-md rounded-lg overflow-hidden text-gray-900 dark:text-gray-200 mt-8 mb-8">
            <div class="p-6">
                ...
                <div class="mt-6 flex justify-center gap-4">
                    @if ($order->status === 'pending')
                        <form action="{{ route('orders.update', $order->id) }}" method="POST">
                            ...
                            <button type="submit"
                                class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600">Process</button>
                        </form>
                    @elseif ($order->status === 'processing')
                        <form action="{{ route('orders.update', $order->id) }}" method="POST">
                            ...
                            <button type="submit"
                                class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600">Complete</button>
                        </form>
                    @endif
                    <a href="{{ route('orders.index') }}"
                        class="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600">Go Back</a>
                </div>
            </div>
        </div>
    </div>
